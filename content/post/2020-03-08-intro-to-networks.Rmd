...
My goal for this post is to show how the basic manipulation steps from the simple example can also be used on much more intense data to easily allow us to carry out a lot of complex calculations with very little code. I would like to note all of this is possible because of the huge effort software developers have put into projects like `igraph`, `tidygraph`, `ggraph`, and `networkD3`.
...
Making networks is much easier than it sounds. We can build one by combining two vectors of equal length. All it takes is two simple vectors of equal length, then just bind them into a dataframe. 
```{r warning=FALSE}
from  <- c(1, 1, 2, 3, 4, 5)
...
The tidygraph object is two dataframes connected. The nodes dataframe (each point) and the edge dataframe (each connection). This structure very human readable and allows for easy inspection of our object. Look below at what is returned when we call the tidygraph object.
```{r messages = FALSE, echo=FALSE}
tidy_basic
```
Tidygraph is powerful because it is a thin wrapper around igraph, and igraph is very very powerful. Tidygraph allows us to use the easily readable code that we are use to from the tidyverse to manipulate igraph objects.


# Working with tidygraph
In order to manipulate the `tbl_graph` we need to tell R which of the two data frames we want to operate on. This is done with `activate(nodes)` or `activate(edges)`. I will show how we would name each node, and give each edge a random amount of transparency. Note the activate call between the operations. 
```{r messages = FALSE}
...
Lets look a the highschool dataset from `ggraph`. This dataset was made by asking a class of students at a all boys highschool to name list their friends in 1957 and then again in 1958. To keep things simple I am going to just use the first year of data. So, lets filter for the first year of data and turn it into a tidygraph.
```{r}
hs_tidy <- highschool %>% 
...
Now lets play around a bit with some of the functionality of ggraph. This package is made to behave a lot like ggplot2. However, one key difference is the layout argument. This tells ggraph what algorithm to use when laying out the nodes. You can go really deep down this rabbit hole. If you want to you can start creating your own layouts with the [particles package](https://cran.r-project.org/web/packages/particles/vignettes/intro.html). 
...
We will leave it there for now, but if you are looking for more check out [this awesome post](https://www.data-imaginist.com/2018/tidygraph-1-1-a-tidy-hope/) from the creator of tidygraph. Or, for a really cool applied example check out [this analysis](https://www.samples-of-thoughts.com/2020/connecting-disinformation-with-tidygraph/) by [@corrieaar](https://twitter.com/corrieaar).